...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (_, file, cb) => {
      ...
      const name = `resume_${Date.now()}_${Math.random()
        .toString(36)
        .slice(2)}${ext}`;
      cb(null, name);
    },
...
async function callOpenAI(resumeText) {
  const completion = await openai.chat.completions.create({
    model: "gpt-3.5-turbo", // kept same as your code
    temperature: 0.2,
    messages: [
...
async function downloadToTemp(urlStr) {
  ...
  if (url.protocol === "file:") {
    ...
    const tmp = path.join(
      UPLOAD_DIR,
      `resume_${Date.now()}_${Math.random().toString(36).slice(2)}${ext}`
    );
  ...
  // HEAD to check size/type
  try {
    const head = await axios.head(urlStr, {
      timeout: 10000,
      maxRedirects: 5,
      validateStatus: () => true,
    });
    ...
    // ignore HEAD failures; GET anyway
  }

  const tmpName = `resume_${Date.now()}_${Math.random()
    .toString(36)
    .slice(2)}${ext || ""}`;
  ...
  try {
    ...
    tryUnlink(tmpPath);
    const e = new Error(
      err && err.message === "File too large" ? "File too large" : "Failed to download file"
    );
...
// ================== MERGED ROUTE ==================
// POST /api/resume
// - Option A: multipart/form-data with field "resume" (PDF/DOCX)
// - Option B: JSON body: { "url": "https://...pdf|docx" } or query ?url=...
router.post(
  "/",
  // Multer will only run on multipart; JSON parser for URL flow
  upload.single("resume"),
  express.json(),
  async (req, res) => {
    let tempFromUrl = null;
    let filePath = null;
    let ext = null;
    ...
    try {
      ...
      if (req.file) {
        ...
        tempFromUrl = await downloadToTemp(url); // { filePath, ext, cleanup }
        filePath = tempFromUrl.filePath;
        ext = tempFromUrl.ext;
      }

      const parsed = await parseResumeFile(filePath, ext);
      return res.json(parsed);
    } catch (err) {
      console.error("Resume parse error:", err);
      if (err.message === "Unsupported file type" || err.code === "UNSUPPORTED")
        return res.status(400).json({ error: "Unsupported file type (only PDF/DOCX)" });
      if (err.code === "PDF_PASSWORD")
      ...
      // cleanup uploaded or downloaded temp file
      try {
        if (tempFromUrl && tempFromUrl.cleanup) tempFromUrl.cleanup();
